...
appendRow.addEventListener("click", ()=>{
    if (getRowCount() < MAX_SIZE) {
        ...
        for(let i = 0; i < getColCount(); i++)newRow.insertCell();
    }
    updateButtonState();
});
removeRow.addEventListener("click", ()=>{
    if (getRowCount() > MIN_SIZE) table.deleteRow(getRowCount() - 1);
    updateButtonState();
});
appendColumn.addEventListener("click", ()=>{
    if (getColCount() < MAX_SIZE) [
        ...table.rows
    ].forEach((row)=>row.insertCell());
    updateButtonState();
});
removeColumn.addEventListener("click", ()=>{
    if (getColCount() > MIN_SIZE) [
        ...table.rows
    ].forEach((row)=>row.deleteCell(getColCount() - 1));
    updateButtonState();
});
